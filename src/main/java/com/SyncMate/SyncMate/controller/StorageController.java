...
import com.SyncMate.SyncMate.dto.responses.file.FileResponse;
...
public class StorageController {
    ...
    public ResponseEntity<FileResponse> uploadFile(@RequestParam("file") MultipartFile file) {
        ...
        FileResponse finalResponse = new FileResponse(true, "Uploaded file successfully", uploadedFile);
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully deleted the file", content = @Content(schema = @Schema(implementation = MakeResponseDto.class))),
    ...
    public ResponseEntity<MakeResponseDto<?>> deleteFile(@PathVariable Long fileId) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        User user = userService.getUserByEmail(authentication.getName());
        boolean deleted = fileService.deleteFile(fileId, user);
        MakeResponseDto<?> finalResponse = new MakeResponseDto<>(true, "File Deleted Successfully", null);
        return ResponseEntity.ok(finalResponse);
    }
